...
namespace CommForAdolph.HtmlHelper
{
    /// <summary>
    /// Html操作相关类
    /// </summary>
    public class HtmlHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string WebClient_Get(string url, Encoding codingType)
        {
            if (codingType == null)
                codingType = Encoding.UTF8;
            WebClient wbClient = new WebClient();
            wbClient.Encoding = codingType;
            byte[] results = wbClient.DownloadData(url);

            return codingType.GetString(results);
        }
